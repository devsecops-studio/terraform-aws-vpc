...
resource "aws_subnet" "ecs" {
  ...
  assign_ipv6_address_on_creation                = var.ecs_subnet_assign_ipv6_address_on_creation
  availability_zone                              = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) > 0 ? element(local.azs, count.index) : null
  availability_zone_id                           = length(regexall("^[a-z]{2}-", element(local.azs, count.index))) == 0 ? element(local.azs, count.index) : null
  cidr_block                                     = element(concat(local.ecs_subnets, [""]), count.index)
  enable_dns64                                   = var.enable_ipv6 && var.ecs_subnet_enable_dns64
  enable_resource_name_dns_aaaa_record_on_launch = var.enable_ipv6 && var.ecs_subnet_enable_resource_name_dns_aaaa_record_on_launch
