locals {
  ...
  enable_ipv6_native_lb_external = var.enable_ipv6 && (var.ipv6_native || var.lb_external_ipv6_native)
}
...
resource "aws_subnet" "lb_external" {
  ...
  cidr_block                                     = local.enable_ipv6_native_lb_external ? null : element(concat(local.lb_external_subnets, [""]), count.index)
  enable_resource_name_dns_a_record_on_launch    = !local.enable_ipv6_native_lb_external && var.lb_external_subnet_enable_resource_name_dns_a_record_on_launch
  enable_resource_name_dns_aaaa_record_on_launch = var.enable_ipv6 && var.enable_resource_name_dns_aaaa_record_on_launch
  private_dns_hostname_type_on_launch            = !local.enable_ipv6_native_lb_external ? var.private_dns_hostname_type_on_launch : "resource-name"

  assign_ipv6_address_on_creation = local.enable_ipv6_native_lb_external ? true : false
  ...
  ipv6_native                     = local.enable_ipv6_native_lb_external
...
resource "aws_route" "lb_external_internet_gateway" {
  count = local.create_lb_external_route_table && !var.ipv6_native && var.create_igw ? 1 : 0
...
resource "aws_route" "lb_external_nat64" {
  count = local.create_lb_external_route_table && var.enable_ipv6 && var.enable_nat_gateway ? var.single_nat_gateway ? 1 : local.nat_gateway_count : 0
