...
resource "aws_route_table" "lb_external" {
  count = local.create_lb_external_route_table ? local.nat_gateway_count : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.lb_external_subnet_suffix}" : format(
        "${var.name}-${var.lb_external_subnet_suffix}-%s",
